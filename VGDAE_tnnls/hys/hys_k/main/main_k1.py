...
def run_model(args, logger, c):
    if args.seed > 0:
        set_rng_seed(args.seed)
    # load data
    train_data, val_data, test_data = dataloader(args)
    #model and optim
    model = eval(args.model)(encoder=Disen_Linear(train_data.x.size(1), args, c)).to(args.device)
...
def main(args):
    ...
    for k in range(1,2):
        ...
        for i in range(args.trails):
            test_auc, test_ap = run_model(args, logger, k)
        ...
        AUC=np.mean(test_auc_list)
        AP=np.mean(test_ap_list)
        k_list_AUC.append(AUC)
        k_list_AP.append(AP)

        logger.info('{}'.format(k_list_AUC))
        logger.info('{}'.format(k_list_AP))
