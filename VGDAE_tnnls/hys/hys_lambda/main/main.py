...
def run_model(args, logger, alpha):
    if args.seed > 0:
...
def main(args):
    ...
    for alpha in np.arange(0.1,1.1,0.1):
        ...
        for i in range(args.trails):
            test_auc, test_ap = run_model(args, logger, alpha)
        ...
        alpha_list_AUC.append(AUC)
        alpha_list_AP.append(AP)

        logger.info('{}'.format(alpha_list_AUC))
        logger.info('{}'.format(alpha_list_AP))
