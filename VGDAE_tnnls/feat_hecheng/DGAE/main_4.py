...
def get_args():
    ...
    parser.add_argument('--q', type=float, default=0.00000002, help='probability of linking between communities')

    parser.add_argument('--p1', type=float, default=0.02, help='probability of linking within 1st communities')
    parser.add_argument('--p2', type=float, default=0.04, help='probability of linking within 2nd communities')
    parser.add_argument('--p3', type=float, default=0.06, help='probability of linking within 3rd communities')
    parser.add_argument('--p4', type=float, default=0.08, help='probability of linking within 4th communities')
    parser.add_argument('--p5', type=float, default=0.1, help='probability of linking within 4th communities')
...
def run_model(args, logger):
    ...
    for epoch in range(args.epoch):
        ...
        for k in range(args.mi_iter):
            ...
            mi_optim.step()
        if epoch%10==0:
            torch.save(model.state_dict(), '/home/dell/zxj/feat/main_4/'+f'syn_data_epoch_{epoch}.pth')
        #evaluation
        model.eval()
        with torch.no_grad():
