...
def get_args():
    ...
    parser.add_argument('--q', type=float, default=0.0000001, help='probability of linking between communities')
...
def run_model(args, logger):
    ...
    for epoch in range(args.epoch):
        ...
        for k in range(args.mi_iter):
            ...
            mi_optim.step()
        if epoch % 50 == 0:
            torch.save(model.state_dict(), '/home/dell/zxj/feat/main_3/' + f'syn_data_epoch_{epoch}.pth')
        #evaluation
        model.eval()
        with torch.no_grad():
