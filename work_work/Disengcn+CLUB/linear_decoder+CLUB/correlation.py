...
class CLUBSample(nn.Module):  # Sampled version of the CLUB estimator
    def __init__(self, hyperpm):
        ...
        self.p_mu = nn.Sequential(nn.Linear(hyperpm['x_dim'], hyperpm['hidden_dim'] // 2),
                                  nn.ReLU(),
                                  nn.Linear(hyperpm['hidden_dim'] // 2, hyperpm['y_dim']))

        self.p_logvar = nn.Sequential(nn.Linear(hyperpm['x_dim'], hyperpm['hidden_dim'] // 2),
                                      nn.ReLU(),
                                      nn.Linear(hyperpm['hidden_dim'] // 2, hyperpm['y_dim']),
                                      nn.ReLU())
