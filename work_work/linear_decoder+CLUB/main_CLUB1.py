...
def train():
    ...
    loss = recon_loss + alpha*total_cor
    ...
    for k in range(hyperpm['mi_iter']):
        ...
        model.eval()
        z = model.encode(train_data.x, train_data.edge_index, hyperpm)
        total_lld_loss, lld_loss, mi_count = 0, 0, 0
        for i in range(hyperpm['k']):
            for j in range(i + 1, hyperpm['k']):
...
# for model_lr in torch.linspace(0.001, 0.01, 10):
for alpha in torch.linspace(0.1, 1, 10):
    ...
    for i in range(hyperpm['run_num']):
        ...
        model = eval(hyperpm['model'])(encoder=Disen_Linear(train_data.x.size(1), hyperpm), decoder=max_deco(hyperpm)).to(device)
        print(model)
        ...
        print(f'alpha:{alpha}, test_auc:{test_auc:.6f}, test_ap:{test_ap:.6f}')
